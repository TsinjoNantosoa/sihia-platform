...
export default function Composer({
  ...
  async function transcribeBlob(blob: Blob) {
    if (!apiBaseUrl || !getAuthHeaders) {
      setVoiceError(
        language === "fr" ? "API chatbot non configurée" : "Chatbot API not configured",
      );
      return;
    }
    ...
    try {
      ...
      setVoiceError(
        language === "fr" ? "Échec de la transcription vocale." : "Voice transcription failed.",
      );
    } finally {
  ...
  async function toggleRecording() {
    ...
    if (!mime) {
      setVoiceError(
        language === "fr" ? "Enregistrement audio non supporté." : "Audio recording not supported.",
      );
      return;
    }
