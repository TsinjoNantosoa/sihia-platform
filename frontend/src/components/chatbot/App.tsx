...
function resolveClientId(): string {
  ...
  return (
    import.meta.env.VITE_CLIENT_ID?.trim() || readRuntimeVar("__CHATBOT_CLIENT_ID__") || "sihia"
  );
}
...
export default function App() {
  ...
  const apiToken = (sessionToken || resolveApiToken()).trim();
  ...
  useEffect(() => {
    ...
    fetch(`${apiBaseUrl}/ui-config?client_id=${encodeURIComponent(clientId)}`, {
      ...
      .catch(() => {
        /* keep slug-based fallback from buildInitialTheme */
      });
